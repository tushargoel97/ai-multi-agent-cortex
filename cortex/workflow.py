...
_DEEP_RESEARCH_DIRECTIVE = (
    ...
    "When fiat conversion is requested, call fiat_exchange_rate with the "
    "actual amount and use its converted value; if another tool supplies the "
    "amount, convert it in the next batch and never calculate it yourself. "
    "Never use crypto_price for fiat. For a retailer-specific product "
    "price, call product_prices with that retailer and region in the first "
    "batch. Accept a retailer price only from that retailer's domain; third-"
    "party aggregators are cross-checks, not proof of its price. Keep sale "
    "price, crossed-out list price, bundles, sellers, and variants separate. "
    "Never put an unverified historical estimate into a result table or trend "
    "calculation. Once useful evidence exists, "
