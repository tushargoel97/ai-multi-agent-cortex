...
/** Pick a cheap, fast model to title with: prefer small/mini models, then the
 *  default, and skip local endpoints (which may be offline). */
...
/** Strip quotes / prefixes / trailing punctuation and clamp length. */
...
async function generate(row: ModelRow, text: string): Promise<string> {
  ...
  // openai / azure_openai (OpenAI-compatible chat completions)
...
/** POST { text } -> { title }. Best-effort: returns an empty title on any
 *  failure so the caller can fall back to a heuristic. */
