#!/usr/bin/env bash
# One-time setup for the trainer: vendors llama.cpp for its HF-to-GGUF converter.
# Usage: bash trainer/setup.sh [llama.cpp git ref]
# Pin a release tag for reproducible builds; the compatibility default is master.
set -euo pipefail
